...
folders = [p for p in raw_results_path.iterdir() if p.is_dir()]
folders = sorted(folders)
last_results = folders[-n_results:]
last_results_names = [p.name for p in last_results]

for folder in last_results_names:
    # Paste here the directory of the h5 file
    file_path = raw_results_path / f"{folder}/optimization_results.h5"
    ...
    df_balanceTechs=df_balanceTechs.drop(columns=['export_price', 'import_price','network_consumption','technology_inputs','technology_outputs'])

    df_balanceTechs=df_balanceTechs.rename(columns={'Photovoltaic_electricity_output':'PV_out',
        'Photovoltaic_curtailment_electricity':'PV_curtailment',
        ...
        'WindTurbine_Onshore_1500_curtailment_electricity': 'Wind_curtailment',
        ...
        'GasTurbine_simple_electricity_output':'GasTurbine_out',
        'generic_production':'RunOfRiver_out',
    })
    ...
    df_balanceTechs["PV_curtailment"]=-df_balanceTechs["PV_curtailment"]
    df_balanceTechs["Wind_curtailment"]=-df_balanceTechs["Wind_curtailment"]
    ...
    colormap = {
        ...
        'GasTurbine_out': '#8B0000', # dark red
    ...
    report.add_figure(plt.gcf(), comment="Energy Balance of the average day for each month")
    ...
    # 1. Calculation & Sorting by Total Volume (Impact)
    df_curt=df_balanceTechs.groupby("ZONE").agg({'PV_curtailment':'sum', 'PV_maxOut':'sum', 'Wind_curtailment':'sum', 'Wind_maxOut':'sum'}).reset_index()
    scale = 1e6
    df_curt['PV_curt_TWh'] = df_curt['PV_curtailment'].abs() / scale
    df_curt['Wind_curt_TWh'] = df_curt['Wind_curtailment'].abs() / scale
    df_curt['PV_max_TWh'] = df_curt['PV_maxOut'] / scale
    df_curt['Wind_max_TWh'] = df_curt['Wind_maxOut'] / scale

    df_curt["Total_Curt_TWh"] = df_curt["PV_curt_TWh"] + df_curt["Wind_curt_TWh"]
    df_curt["Total_Max_TWh"] = df_curt["PV_max_TWh"] + df_curt["Wind_max_TWh"]
    df_curt["Curtailement_Percent"] = (df_curt["Total_Curt_TWh"] / df_curt["Total_Max_TWh"]) * 100
    # Convert to categorical with the specified order
    df_curt['ZONE'] = pd.Categorical(df_curt['ZONE'], categories=desired_order, ordered=True)
    # Sort by that category.
    # We sort descending=False so that 'NORD' (the first item) appears at the TOP of the horizontal chart.
    df_curt = df_curt.sort_values("ZONE", ascending=False)

    # 2. Plotting
    fig, ax = plt.subplots(figsize=(12, 8))
    y_range = np.arange(len(df_curt))
    color_pv = '#ffd700'
    color_wind = '#7fc97f'

    # --- THE "POTENTIAL" GHOST BARS (The background container) ---
    ax.barh(y_range, df_curt['PV_max_TWh'], color=color_pv, alpha=0.15, label='PV Potential')
    ax.barh(y_range, df_curt['Wind_max_TWh'], left=df_curt['PV_max_TWh'], color=color_wind, alpha=0.15, label='Wind Potential')

    # --- THE "CURTAILED" SOLID BARS (The waste next to each other) ---
    # PV curtailed starts at 0
    ax.barh(y_range, df_curt['PV_curt_TWh'], color=color_pv, edgecolor='black', linewidth=0.5, label='PV Curtailed')
    # Wind curtailed starts at the end of PV CURTAILED (this puts them side-by-side)
    ax.barh(y_range, df_curt['Wind_curt_TWh'], left=df_curt['PV_curt_TWh'], color=color_wind, edgecolor='black', linewidth=0.5, label='Wind Curtailed')

    # 3. Percentage Labels (Now at the end of the SOLID bars)
    max_x_view = df_curt['Total_Max_TWh'].max()
    for i, (pct, total_curt) in enumerate(zip(df_curt['Curtailement_Percent'], df_curt['Total_Curt_TWh'])):
        ax.text(total_curt + (max_x_view * 0.01), i, f"{pct:.1f}%",
                va='center', fontweight='bold', color='#333333')

    # Aesthetics
    ax.set_yticks(y_range)
    ax.set_yticklabels(df_curt['ZONE'], fontsize=10, fontweight='bold')
    ax.set_xlabel('Energy Volume (TWh)', fontsize=12)
    ax.set_title('Nodal Curtailment Impact: Lost Volume vs. Full Capacity', fontsize=16, pad=20, loc='left')
    ...
    plt.tight_layout()
    report.add_figure(fig, comment="Curtailment of PV and wind in TWh with relative capacity ghosts")
    ...
    ita_row = pd.DataFrame({
        ...
        'GasTurbine_out': [df_maxout['GasTurbine_out'].sum()],
    ...
    #Plot 6: Energy mix annuale complessivo
    with h5py.File(file_path, 'r') as hdf_file:
        technologies=[]
        for node in nodes:
    ...
    exclude_cols = {
        ...
        'PV_out', 'Wind_out', 'day_of_month', 'network_inflow', 'network_outflow', 'PV_curtailment', 'Wind_curtailment'
    }
